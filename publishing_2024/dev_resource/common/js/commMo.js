...
2024.10.10  이상봉      1.1.3       전자약정 streamDocs 적용
*********************************************************/
    ...
    isValidCustNo: function(custNo) {    	
    	if(custNo == "" || custNo == undefined || custNo == null){
    		CommMsg.msgPop(
	            ...
	            function(){
	                const url = "/login/appAuthLogin.nh";
	                comm.goPage(url);
            });
    	}else{
    		return true;
    ...
    */
    ...
    terms: {
        ...
        fn_openPopupTermsDetail: (callback) => {
            ...
            let { TITLE, FILE_NAME } = commMo.terms.fileNameList[0];
...
const fn_edit_my_information_mo = function(){
    ...
    comm.callAjaxJson(url,{},function(data){
    	...
    	if(success === "E"){
    		...
    		// 전자서명 인증
    		fn_certify_electronic_sign_mo(editInfo);
    	}
    });
}
 ...
 */
const fn_certify_electronic_sign_mo = function(editInfo) {
	const editInfoArr = $.map(editInfo, function(value, key) {
    ...
    comm.callAjaxJson(url,sendData,function(data){
        ...
        if(success === "Y"){
            ...
            if(errData != "" || errData != null || errData != 'undefined'){
                ...
                const msgInfo = msgGroup.pop(0);
                const errMsg = msgInfo['RSP_MSG_AD'];
                CommMsg.msgPop({type:"a",icon:"Y",cont:errMsg},function(){return false;});
            }
        }
    });
}
        ...
        if(resultCd === "S"){
            ...
            switch(prgsStcd){
                case "000" :
                	...
                	// 2024.10.22 24Hour/365Day 구축 프로젝트. 이상봉. 플랫폼 한도조회여부 체크
                	if (rtnData.PLTF_IQRY_YN_S1 != "Y") {
                		comm.goPage("/eloan/scraping/firstInquiry.nh");
                	} else {
                		// 제휴 플랫폼에서 한도조회를 한 경우 한도조회 결과 화면으로 이동함
                		const param = {
                			custNo     : rtnData.CUSTNO,
                			consNo     : rtnData.CONS_NO,
                			pltfIqryYn : "Y"
                		}
                		
                		comm.goPage("/eloan/credit/selectCreditLoanLimitResult.nh", param);
                	}
                ...
                case "100" :
                    ...
                    if(prgsStep === "01"){
                    	...
                    	// 2024.10.04 24Hour/365Day 구축 프로젝트. 이상봉. 이어하기 금소법으로 변경. 금소법부터는 앱어서만 가능
                        stepNm = "적합성·적정성 진단";
                        ...
                        if (rtnData.chkChannel === "MA") {
                        	...
                        	stepUrl = "/eloan/credit/selectCustProtection.nh";
                        } else {
                        	// stepUrl = "/eloan/credit/selectCreditLoanApplyFinish.nh";
                        	stepUrl = "/eloan/credit/selectCustProtection.nh";
                        }
...
let fn_callback_do_cert_sign = ()=>{
	...
	if(midmenu == "mypg"){
		...
		// 대리중개인 여부(신규, 중고, 리스)
		if(mid != "ltrc") sendData['agncBrokNtfGdncYn'] = document.querySelector("#chk99_0")?.checked?"Y":"N";	
	}
...
// 2024.10.16 24Hour/365Day 구축 프로젝트. 이상봉. 전자약정 본인인증 반영 start
const fn_callback_elecAgreHpAuthIcln1 = function() {
	const url = "/eloan/credit/selectElecAgreHpAuth.nh";
    
	comm.callAjaxJson(url, {}, function(data) {
        if(data.resultCode === "S"){
        	comm.goPage("/eloan/credit/insertCreditLoanPFCInfo.nh");
        } else {
        	CommMsg.msgPop(
        		{type:"a",icon:"Y",cont:data.resultMsg},
            	function() {
        			comm.goPage('/home/homemain.nh');
        		}
        	);
...
const fn_callback_elecAgreHpAuthIcln2 = function() {
	const url = "/eloan/credit/selectElecAgreHpAuth.nh";
    
	comm.callAjaxJson(url, {}, function(data) {
        if(data.resultCode === "S"){
        	comm.goPage("/eloan/credit/selectCreditLoanContractDetail.nh");
        } else {
        	CommMsg.msgPop(
            	{type:"a",icon:"Y",cont:data.resultMsg},
                function() {
...
const fn_callback_elecAgreHpAuthCar = function() {
	const url = "/eloan/carfinance/car/selectElecAgreHpAuth.nh";
	
	comm.callAjaxJson(url, {}, function(data) {
        if(data.resultCode === "S"){
        	comm.goPage("/eloan/carfinance/car/selectApplyInfo.nh");
        } else {
        	CommMsg.msgPop(
        		{type:"a",icon:"Y",cont:data.resultMsg},
            	function() {
...
const fn_callback_elecAgreHpAuthConstruct = function() {
	const url = "/eloan/carfinance/car/selectElecAgreHpAuthCons.nh";

	comm.callAjaxJson(url, {}, function(data) {
        if(data.resultCode === "S"){
        	comm.goPage("/eloan/carfinance/construct/selectApplyInfoList.nh");
        } else {
        	CommMsg.msgPop(
        		{type:"a",icon:"Y",cont:data.resultMsg},
        		function() {
...
//2024.10.16 24Hour/365Day 구축 프로젝트. 이상봉. 전자약정 본인인증 반영 end

//메인 전자약정 휴대폰인증
const fn_call_ajax_hpAuth_onlineContract = function(){	
    const url = "/home/selectHpAuthOnlineContract.nh";
    const sendData = {};
    
    comm.callAjaxJson(url,sendData,function(rtnData){

        const resultCd = rtnData.resultCode;
        const custNo = rtnData.custNo;
        //alert("삭제예정1 : "+resultCd+", "+custNo);
        if(resultCd === "S"){
            const subUrl = "/home/selectOnlineContractCnt.nh";      	
            const subSendData = {custNo:custNo};
            comm.callAjaxJson(subUrl,subSendData,function(subRtnData){
            	const subResultCd = subRtnData.resultCode;
            	const subCarCnt = subRtnData.carCnt;
            	const subCreditCnt = subRtnData.creditCnt;
            	const subConCnt = subRtnData.conCnt;
            	if(subResultCd==="S"){
                    //alert("삭제예정2 : "+subCarCnt+", "+subCreditCnt+", "+subConCnt);
            		if(subCarCnt == "0" && subCreditCnt == "0" && subConCnt == "0" ){
            			goOnlineContractMainlPage("NONE");
            		} else {
            			goOnlineContractMainlPage("Y");
            			//comm.goPage("/home/selectOnlineContractPopForm.nh?custNo="+custNo);
            		}
            	}else {
                    const subResultMsg = subRtnData.resultMsg;
                    CommMsg.msgPop({type:"a",icon:"Y",cont:subResultMsg});
                    return false;
            	}
            });
        }else{
...
function goOnlineContractMainlPage(onlineContractYn) {
	var param = {
			onlineContractYn: 		 onlineContractYn
	};
	comm.goPage("/home/homemain.nh", param);
}
const fn_callback_elecAgreHpAuthStockLoanIdCheck = function() {
    const url = "/home/selectHpAuthOnlineContract.nh";
	//const url = "/eloan/carfinance/car/selectElecAgreHpAuth.nh";
	comm.callAjaxJson(url, {}, function(data) {
        if(data.resultCode === "S"){
        	comm.goPage("/eloan/stock/selectStockLoanIdCheck.nh");///eloan/stock/selectStockLoanIdCheck.nh    /eloan/stock/selectApplyInfoStockLoanIdCheck.nh
        } else {
        	CommMsg.msgPop({type:"a",icon:"Y",cont:data.resultMsg});
        }
    });
}