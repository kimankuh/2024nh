...
var comm = {
    ...
    ************************************************************************/
    ...
    stockShowLoadingMW: function(userName) {
        ...
        msglayer +='                <div class="title_box">';
        // 2024.08.23 24Hour/365Day 구축 프로젝트. 이상봉. 문구 변경
    ...
    ************************************************************************/
    goPage : function(url,param){
        ...
        } else if (url == "/login/appAuthLogin.nh"){
        	if (document.location.host == "mp.nhcapital.co.kr"){
        		url = "https://m.nhcapital.co.kr/login/appAuthLogin.nh";
        	} else if (document.location.host == "tmp.nhcapital.co.kr"){
        		url = "https://tm.nhcapital.co.kr/login/appAuthLogin.nh";
        	}        	
        }
        $("#goPageFrom").remove();
...
*/
    ...
    ************************************************************************/
    ...
    // 2024.09.06 24Hour/365Day 구축 프로젝트. 이상봉. SMS 해시값 가져오는 함수 추가
    /************************************************************************
    @함수명        : callNativeSmsAppHash
    @설 명        : SMS 해시값 가져오기(안드로이드)
    @사용법        : comm.callNativeSmsAppHash() 호출
    @작성일        : 2024-09-06
    @작성자        : 이상봉
     ...
     ************************************************************************/
    callNativeSmsAppHash : function (callback) {
        if((typeof callback) != "function"){
            comm.callNativeFunc("SMS_APP_HASH", {}, callback);
        } else{
            comm.callNativeIfc("SMS_APP_HASH", {}, callback);
        }
    },
    
    // 2024.09.06 24Hour/365Day 구축 프로젝트. 이상봉. SMS 해시값 가져오는 함수 추가
    /************************************************************************
    @함수명        : callNativeReadSmsCode
    @설 명        : SMS인증번호 받기(안드로이드)
    @사용법        : comm.callNativeReadSmsCode() 호출
    @작성일        : 2024-09-06
    @작성자        : 이상봉
     ...
     ************************************************************************/
    callNativeReadSmsCode : function (callback) {
        if((typeof callback) != "function"){
            comm.callNativeFunc("READ_SMS_CODE", {}, callback);
        } else{
            comm.callNativeIfc("READ_SMS_CODE", {}, callback);
        }
    },
    
    // 2024.09.06 24Hour/365Day 구축 프로젝트. 이상봉. 갤러리에서 이미지 가져오는 함수 추가
    /************************************************************************
    @함수명        : callNativeOpenGallery
    @설 명        : 갤러리에서 이미지 가져오기
    @사용법        : comm.callNativeOpenGallery() 호출
    @작성일        : 2024-09-06
    @작성자        : 이상봉
     ...
     ************************************************************************/
    callNativeOpenGallery : function (callback) {
        if((typeof callback) != "function"){
            comm.callNativeFunc("OPEN_GALLERY", {}, callback);
        } else{
            comm.callNativeIfc("OPEN_GALLERY", {}, callback);
        }
    },
...
//        $("#"+layerId).find("div.ty_error").hide();
    ...
    */
    ...
    showProgress : function(time){
    	...
    	// 2024.09.05 24Hour/365Day 구축 프로젝트. 이상봉. 문구 수정
  		...
  		// 2024.08.22 24Hour/365Day 구축 프로젝트. 이상봉. 문구 수정
...
/*    	if (_ua.toLocaleLowerCase().indexOf("kakaotalk") > -1) {
    ...
    // 계좌점유인증 요청
    //param: bankCode:은행코드, account:계좌번호, popupYN:계좌점유인증 인증번호 팝업 호출여부, loadingbarYN: 로딩바 호출여부
    //popOption: 인증번호 팝업사용시 Option 정보
    //callbackSucFunc: 계좌점유인증 요청 성공시 callback
    //callbackErrFunc: 계좌점유인증 요청 실패시 callback
    //callbackPopupFunc: 계좌점유인증 인증번호 팝업 종료시 callback(param.popupYN: "N"이 아닌 경우, 성공/실패 callback)
    doAccAuthReq : function(param, popOption, callbackSucFunc, callbackErrFunc, callbackPopupFunc) {
    	var sendData1 = {
    	        bankCode: typeof param.bankCode != 'undefined' ? param.bankCode : "",
    	        account: typeof param.account != 'undefined' ? param.account : "",
    	        CONS_NO: typeof param.CONS_NO != 'undefined' ? param.CONS_NO : "",
    	        CUST_NO: typeof param.CUST_NO != 'undefined' ? param.CUST_NO : ""
    		};
    	
    	var ajaxOption = {};
    	if(param.loadingbarYN == "N") ajaxOption.dimType="none";
    	
    	//계좌점유인증 요청
    	comm.callAjaxJson("/comm/auth/accAuthRequest.nh", sendData1, function(result1) {
    		if(result1.resultType == "S") {
    			var sendData2 = {
    					userName: result1.userName,
    					userBirthday: result1.userBirthday,
    					bankCode: result1.bankCode,
    					account: result1.account
    			};
    			
    			
    			if(param.loadingbarYN != "N") comm.showLoading();
    			//계좌점유인증(No UI) API.. 3번째 parameter설정하지 않는 경우(undefined) 별도의 인증코드 확인 API를 통해서 검증가능.
    			CertSign.accountAuthNoUI(result1.siteKey, sendData2, undefined, function(result2) {
    				if(param.loadingbarYN != "N") comm.hideLoading();
    				
    				result2.accAuthApi = "CertSign.accountAuthNoUI";
    				comm.callAjaxJson('/comm/auth/ajaxAccAuthLgAd.nh', result2, function(axReq) {
    					if(axReq.resultType == "S") {
    						if(result2.resultCode === 0) {
    	    					comm.callAjaxJson("/comm/auth/ajaxAccAuth.nh", {"txToken" : result2.txToken}, function(result3) {
    	    						if(result3.resultType == "S") {
    	    							if(callbackSucFunc) callbackSucFunc(result3);
    	    							
    	    							//계좌점유인증 인증번호 팝업 호출일시
    	    							if(param.popupYN !="N") {
    	    								// 계좌점유인증 인증번호 팝업 호출
    	    								comm.doAccAuthCodePopup(popOption, callbackPopupFunc);
    	    							}
    	    							
    	    						}else {
    	    							result3.resultCode = "NHCERR";
    	    							if(callbackErrFunc) callbackErrFunc(result3);
    	    						}
    	    					}, ajaxOption);
    	    					
    	    				}else {
    	    					if(result2.resultCode == "2000") {
    	    						result2.resultMsg = "계좌번호를 다시 확인해주세요.";
    	    							
    	    					}else if(result2.resultCode == "2501") {
    	    						result2.resultMsg = "1원 송금을 통한 결제계좌 등록을 위한 인증코드 확인 횟수를 초과 했어요.";
    	    						
    	    					}else if(result2.resultCode == "1105" || result2.resultCode == "1106") {
    	    						result2.resultMsg = "사용자를 찾을 수 없습니다.<br/>사용자정보(은행, 계좌번호 등)를 확인 후 다시 시도해 주세요.";
    	    						
    	    					}else if(result2.resultCode == "4000") {
    	    						result2.resultMsg = "서버 통신 상태가 원활하지 않습니다.<br/>잠시후 다시 시도해 주세요.";
    	    						
    	    					}else {
    	    						result2.resultMsg = result2.resultMessage;
    	    					}
    	    					
    	    					//edge에서 undefined로 1~3번 callback 되는 경우가 있음...
    	    					if(typeof result2.resultCode != 'undefined') {
    	    						if(callbackErrFunc) callbackErrFunc(result2);
    	    					}
    	    				}
    						
    					}else {
    						axReq.resultCode = "NHCERR";
							if(callbackErrFunc) callbackErrFunc(axReq);
    					}
    					
    				}, ajaxOption);
    			});
    			
			}else if(result1.resultType == "L") {
				if(callbackSucFunc) callbackSucFunc(result1);
				...
				result1.resultCode = "NHCERR";
				if(callbackErrFunc) callbackErrFunc(result1);
			}
    	}, ajaxOption);
    ...
    // 계좌점유인증 인증번호 팝업 호출
    doAccAuthCodePopup : function(option, callbackFunc) {
        var url = "/comm/auth/accAuthCodePopupForm.nh";
        var id = "accAuthCodePopupForm";
        CommPop.layerPopup(id, url, option, callbackFunc);
    },
    
    // 계좌점유인증 인증번호 확인
    doAccAuthCodeConfirm : function(param, callbackSucFunc, callbackErrFunc) {
    	var ajaxOption = {};
    	if(param.loadingbarYN == "N") ajaxOption.dimType="none";
    	
    	comm.callAjaxJson("/comm/auth/accAuthConfirm.nh", {"authCode": param.authCode}, function(result1) {
    		if(result1.resultType == "S") {
    			var info = {
    					txToken: result1.txToken,
    					authCode: result1.authCode
    			};
    			
    			if(param.loadingbarYN != "N") comm.showLoading();
    			// 계좌점유인증 인증코드 확인 API
    			CertSign.verifyAccountAuthCode(result1.siteKey, info, function(result2) {
    				if(param.loadingbarYN != "N") comm.hideLoading();
    				
    				result2.accAuthApi = "CertSign.verifyAccountAuthCode";
    				comm.callAjaxJson('/comm/auth/ajaxAccAuthLgAd.nh', result2, function(axReq) {
    					if(axReq.resultType == "S") {
    						if(result2.resultCode === 0) {
    	    		            comm.callAjaxJson("/comm/auth/ajaxAccAuth.nh", {"txToken" : result2.txToken}, function(result3) {
    ...
    //계좌점유인증 검증
    doAccAuthCodeVerify : function(param, callbackSucFunc, callbackErrFunc) {
    	var ajaxOption = {};
    	if(param.loadingbarYN == "N") ajaxOption.dimType="none";
    	
    	comm.callAjaxJson("/comm/auth/accAuthVerify.nh", {}, function(result1) {
    		if(result1.resultType == "L") {
    			if(callbackSucFunc) callbackSucFunc(result1);
    			
    		}else {
    			result1.accAuthApi = "accAuthVerify";
    			comm.callAjaxJson('/comm/auth/ajaxAccAuthLgAd.nh', result1, function() {}, ajaxOption);
    			if(result1.resultType == "S") {
    				if(callbackSucFunc) callbackSucFunc(result1);
    				
    			}else {
    				result1.resultMsg = result1.resultMessage;
    				if(callbackErrFunc) callbackErrFunc(result1);
    			}
    		}
        }, ajaxOption);
